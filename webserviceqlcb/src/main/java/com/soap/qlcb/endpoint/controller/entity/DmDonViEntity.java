 ...
 */
@Entity
@Table(name = "DM_DON_VI", schema = "QLTB", catalog = "")
@SqlResultSetMappings({
        @SqlResultSetMapping(name="groupDetailsMapping",
                classes={
                        @ConstructorResult(
                                targetClass=DmDonViEntityResult.class,
                                columns={
                                        @ColumnResult(name="ID",type=long.class),
                                        @ColumnResult(name="MA_DON_VI",type=String.class),
                                        @ColumnResult(name="TEN_TIENG_VIET",type = String.class),
                                        @ColumnResult(name="TINH_THANH_ID",type = Long.class),
                                        @ColumnResult(name="DIA_CHI",type = String.class),
                                        @ColumnResult(name="DONVI_CHA_ID",type = Long.class),
                                        @ColumnResult(name="CAP_DON_VI",type = Long.class)
                                }
                        )
                }),
        @SqlResultSetMapping(name="mappingDmDonVi",
                classes={
                        @ConstructorResult(
                                targetClass=ResultDmDonVi.class,
                                columns={
                                        @ColumnResult(name="ID",type=long.class),
                                        @ColumnResult(name="MA_DON_VI",type=String.class),
                                        @ColumnResult(name="TEN_TIENG_VIET",type = String.class),
                                        @ColumnResult(name="TEN_TIENG_ANH",type = String.class),
                                        @ColumnResult(name="TINH_THANH_ID",type = Long.class),
                                        @ColumnResult(name="TEN_TINH_THANH",type = String.class),
                                        @ColumnResult(name="DIA_CHI",type = String.class),
                                        @ColumnResult(name="DONVI_CHA_ID",type = Long.class),
                                        @ColumnResult(name="GD",type = Integer.class),
                                        @ColumnResult(name="PGD",type = Integer.class),
                                        @ColumnResult(name="NV",type = Integer.class)
                                }
                        )
                })
})
...
@NamedNativeQueries({
        @NamedNativeQuery(
                name="DmDonViEntity.myProc",
                query="SELECT dv.ID, dv.MA_DON_VI,dv.TEN_TIENG_VIET,dv.TINH_THANH_ID,dv.DIA_CHI,dv.DONVI_CHA_ID,dv.CAP_DON_VI from QLCB2.DM_DON_VI dv ",
                resultSetMapping="groupDetailsMapping"
        ),
        @NamedNativeQuery(
                name="DmDonViEntity.getDmDonViByNativeQuery",
                query="select a.id,a.MA_DON_VI,a.TEN_TIENG_VIET,a.TEN_TIENG_ANH,a.TINH_THANH_ID,dmtt.TEN_TINH_THANH,a.DIA_CHI,a.DONVI_CHA_ID,tem1.cv as GD,tem3.cv as PGD ,tem2.cv as NV  from QLCB2.DM_DON_VI a" +
//                query="select dv.id,dv.MA_DON_VI,dv.TEN_TIENG_VIET,dv.TINH_THANH_ID,dv.DIA_CHI,dv.DONVI_CHA_ID,dv.CAP_DON_VI from QLCB2.DM_DON_VI dv" +
                        " left join QLCB2.DM_TINH_THANH dmtt on a.TINH_THANH_ID=dmtt.ID" +
                        " left join" +
                        "(select a.ID,b.CHUC_VU_ID,count(b.CHUC_VU_ID) as cv" +
                        " from DM_DON_VI a LEFT join CB_CAN_BO b on a.ID = b.DON_VI_ID left join DM_CHUC_VU c on b.CHUC_VU_ID =c.ID where b.chuc_vu_id in (287) group by b.CHUC_VU_ID,a.ID )" +
                        " tem1" +
                        " on a.id = tem1.id" +
                        " left join" +
                        "(select a.id,ba.cv from DM_DON_VI a left join (select DON_VI_ID,count(*) as cv from CB_CAN_BO where CHUC_VU_ID not in (287,288) or CHUC_VU_ID is null group by DON_VI_ID) ba on a.id =ba.don_vi_id) tem2" +
                        " on a.id = tem2.id" +
                        " left join" +
                        "(select a.id,ba.cv from DM_DON_VI a left join (select DON_VI_ID,count(*) as cv from CB_CAN_BO where CHUC_VU_ID  in (288) group by DON_VI_ID) ba on a.id =ba.don_vi_id) tem3" +
                        " on a.id=tem3.id" +
                        " order by a.id",

                resultSetMapping="mappingDmDonVi"
        )
})
@NamedStoredProcedureQueries({
        @NamedStoredProcedureQuery(name = "DmDonViEntity.procGetDmDonVi",
                procedureName = "dm_don_vi_qltb_select",
//                resultSetMappings = "groupDetailsMapping",
                resultClasses = ResultDmDonVi.class,
                parameters = {
...
public class DmDonViEntity {
    ...
    @Basic
    @Column(name = "TINH_THANH_ID",insertable = false, updatable = false)
    ...
    public boolean equals(Object o) {
        ...
        DmDonViEntity that = (DmDonViEntity) o;
